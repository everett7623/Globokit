...
export default function ExportTaxRebateCalculatorPage() {
  ...
  const copy = async () => { await navigator.clipboard.writeText([`出口退税测算`,`FOB 收入：${money.format(result.fobRevenueCny)}`,`预计退税：${money.format(result.estimatedRebateCny)}`,`采购现金支出：${money.format(result.purchaseCashOutflowCny)}`,`出口费用：${money.format(result.exportExpenseCny)}`,`退税后利润：${money.format(result.profitBeforeTaxCny)}`,`利润率：${result.profitMarginPercent.toFixed(2)}%`].join('\n')); setCopied(true); window.setTimeout(() => setCopied(false), 1800) }
  const cards = [
