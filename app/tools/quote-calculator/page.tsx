...
export default function QuoteCalculatorPage() {
  ...
  const getSummaryText = () => calculation.result ? buildSummary(calculation.result, form.currency, formatForeign) : ''
  ...
  return (
    <MobileFriendlyWrapper>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">结合采购成本、运费、汇率、佣金和目标利润率，按 EXW/FCA/FOB/CFR/CIF/DDP 等条款测算报价区间。</p>
      </div>
      {calculation.error && (
        <EnhancedAlert
          type="error"
      ...
      {calculation.result && <QuoteStats result={calculation.result} formatForeign={formatForeign} />}
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
        <QuoteForm
          form={form}
          mode={mode}
          missingCostLabels={calculation.result?.missingCostLabels ?? []}
          onFieldChange={updateField}
          onModeChange={setMode}
          onTermChange={(quoteTerm: QuoteTerm) => setForm((current) => ({ ...current, quoteTerm }))}
          onCurrencyChange={(currency: QuoteCurrency, exchangeRate: string) => setForm((current) => ({ ...current, currency, exchangeRate }))}
          onPreset={(values, nextMode) => { setMode(nextMode); setForm((current) => ({ ...current, ...values })) }}
          onReset={reset}
        />
        <QuoteResults
