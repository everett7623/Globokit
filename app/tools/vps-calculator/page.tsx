...
export default function VPSCalculatorPage() {
  ...
  const [renewalPeriod, setRenewalPeriod] = useState('36') // 默认三年(36个月)，方便测试你的截图案例
  ...
  const handleCalculate = () => {
    ...
    const expectedNum = expectedPrice === '' ? -1 : parseFloat(expectedPrice) // -1 标记为空
    ...
    const res = calculateVPSValue(
      ...
      priceNum,
      currency,
      expectedNum === -1 ? 0 : expectedNum, // 如果没填，传0进函数，函数内部会处理默认值
      exchangeRates,
      tradeDate
    )
    
    // 如果用户没填期望售价，在这里把期望售价同步为剩余价值，方便UI展示
    if (expectedNum === -1) {
       res.expectedPrice = res.remainingValue
       res.premium = 0
       res.premiumPercent = 0
    }
