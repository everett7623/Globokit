...
export function CountryCommunicationTab({ country, localTimezone }: { country: CountryInfo; localTimezone: string | null }) {
  const difference = localTimezone ? getTimeDifference(country.timezone, localTimezone) : null
  const relativeTime = difference === null ? '--' : `${difference >= 0 ? '+' : ''}${difference}h`
  return <div className="grid gap-4"><CopyRow icon={Phone} label="国际电话区号" value={country.dial_code} large /><CopyRow icon={Hash} label="国家代码" value={`${country.iso2} / ${country.iso3}`} /><CopyRow icon={Globe} label="顶级域名" value={country.tld} /><div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg"><span className="flex items-center gap-3"><Clock className="h-5 w-5 text-muted-foreground" />时区 / 时差</span><div className="text-right"><div className="font-medium">{country.timezone}</div><div className="text-sm text-muted-foreground">{relativeTime}（相对本地）</div></div></div></div>
}
