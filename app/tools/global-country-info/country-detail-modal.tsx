...
export function CountryDetailModal({ country, favorites, localTimezone, onToggleFavorite, onClose }: { country: CountryInfo; favorites: string[]; localTimezone: string | null; onToggleFavorite: (iso2: string) => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"><Card className="w-full max-w-4xl max-h-[90vh] overflow-auto">
    ...
    <CardContent className="p-6"><Tabs defaultValue="basic" className="w-full"><TabsList className="grid w-full grid-cols-4"><TabsTrigger value="basic">基础信息</TabsTrigger><TabsTrigger value="communication">通讯信息</TabsTrigger><TabsTrigger value="culture">文化信息</TabsTrigger><TabsTrigger value="business">商务信息</TabsTrigger></TabsList><TabsContent value="basic" className="space-y-4 mt-6"><CountryBasicTab country={country} /></TabsContent><TabsContent value="communication" className="space-y-4 mt-6"><CountryCommunicationTab country={country} localTimezone={localTimezone} /></TabsContent><TabsContent value="culture" className="space-y-4 mt-6"><CountryCultureTab country={country} /></TabsContent><TabsContent value="business" className="space-y-4 mt-6"><CountryBusinessTab country={country} /></TabsContent></Tabs></CardContent>
  </Card></div>
}
