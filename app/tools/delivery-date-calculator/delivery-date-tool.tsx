...
export function DeliveryDateTool({ defaultStartDate }: { defaultStartDate: string }) {
  ...
  const copySummary = () => {
    ...
    const lines = ['Delivery Lead Time Estimate', `Start Date: ${formatDeliveryDateEnglish(result.startDate)}`, `Lead Time: Production ${form.productionDays} days + Transit ${form.transitDays} days + Buffer ${form.bufferDays} days`, `Calculation Basis: ${form.dayMode === 'business' ? 'Business days' : 'Calendar days'}`, ...result.milestones.map((milestone) => `${milestoneLabels[milestone.key]}: ${formatDeliveryDateEnglish(milestone.date)} (+${milestone.days} days)`), `Weekend Days Excluded: ${result.weekendDaysSkipped}`, `Public Holidays Excluded: ${result.holidayDaysSkipped}`]
    return lines.join('\n')
  }

  return <><div className="mb-8"><h1 className="mb-2 text-3xl font-bold">外贸交期计算器</h1><p className="text-muted-foreground">按生产、运输和缓冲周期推算交付日期，支持自然日、工作日与国家公共假日口径</p></div><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_460px]"><DeliveryDateForm form={form} copyText={copySummary()} canCopy={Boolean(calculation.result)} onStartDate={(startDate) => setForm((current) => ({ ...current, startDate }))} onDayField={updateDayField} onMode={(dayMode: DeliveryDayMode) => setForm((current) => ({ ...current, dayMode }))} onHolidayToggle={() => setForm((current) => ({ ...current, excludeHolidays: !current.excludeHolidays }))} onCountry={(countryCode) => setForm((current) => ({ ...current, countryCode }))} onPreset={(values) => setForm((current) => ({ ...current, ...values }))} onReset={() => setForm(createInitialDeliveryForm(getDateKeyInTimeZone(new Date())))} /><DeliveryDateResults calculation={calculation} /></div></>
}
