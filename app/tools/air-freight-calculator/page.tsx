  ...
  return (
    <MobileFriendlyWrapper>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">按箱规、毛重、净重、泡重系数和运价估算空运快递计费重量、抛重差与费用</p>
      </div>
      <AirFreightStats result={result} />
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <AirFreightForm
          form={form}
          divisorNote={divisorNote}
          onFieldChange={updateField}
          onDivisorChoice={updateDivisorChoice}
          onPreset={(values) => setForm((current) => ({ ...current, ...values }))}
          summaryText={getSummaryText()}
          onReset={() => setForm(INITIAL_FORM)}
        />
        <AirFreightResults result={result} />
      </div>
    </MobileFriendlyWrapper>
  )
}
