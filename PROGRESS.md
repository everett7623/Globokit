...
- 完成剩余工具体验迁移：31 个工具全部接入 `MobileFriendlyWrapper`，复制操作统一使用 `EnhancedCopyButton` 或其兼容包装。
...
- 应用内浏览器仍因 `failed to write kernel assets: 系统找不到指定的路径。 (os error 3)` 无法初始化，滞箱滞港费、包装方案、阶梯报价与保本销量工具的真实点击和 390px 视口待环境恢复后补测。
